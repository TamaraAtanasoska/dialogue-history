...
from pretrain.lxmert_data import (
    InputExample,
    LXMERTDataset,
    LXMERTTorchDataset,
    LXMERTEvaluator,
...
class LXMERT:
    ...
    def forward(self, examples):
        ...
        input_ids = torch.tensor(
            [f.input_ids for f in train_features], dtype=torch.long, device=device
        )
        input_mask = torch.tensor(
            [f.input_mask for f in train_features], dtype=torch.long, device=device
        )
        segment_ids = torch.tensor(
            [f.segment_ids for f in train_features], dtype=torch.long, device=device
        )
        ...
        lm_labels = torch.tensor(
            [f.lm_label_ids for f in train_features], dtype=torch.long, device=device
        )
        ...
        matched_labels = torch.tensor(
            [f.is_matched for f in train_features], dtype=torch.long, device=device
        )
