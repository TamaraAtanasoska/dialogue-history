...
if __name__ == "__main__":
    ...
    with open(args.save_tsv_path, mode="w") as out_file:
        ...
        for i_batch, sample in enumerate(tqdm.tqdm(dataloader, total=len(dataloader))):
            ...
            guesser_accuracy, guesses, guesses_probs = calculate_accuracy_verbose(
                softmax(guesser_out), sample["target_obj"].cuda()
            )
            ...
            for i in range(sample["history"].shape[0]):
                if guesses[i] == 1:
                    masked_objects = [
                        x for x in sample["objects"][i].cpu().tolist() if x != 0
                    ]
                    if len(masked_objects) <= 5:
                        if len(set(masked_objects)) == len(masked_objects):
                            guesses_confidence = guesses_probs[i].item() - 1 / len(
                                set(masked_objects)
                            )
                            dialogs.append(
                                (
