...
def create_data_file(data_dir, data_file, data_args, vocab_file_name, split="train"):
    ...
    ans2tok = {"Yes": word2i["<yes>"], "No": word2i["<no>"], "N/A": word2i["<n/a>"]}
        ...
        for json_game in file:
            ...
            for i, qa in enumerate(game["qas"]):
                if i != 0:
                    ...
                    src_unpad = (
                        src[: src.index(word2i["<padding>"])]
                        if word2i["<padding>"] in src
                        else src
                    )
                    target_q_unpad = (
                        target_q[: target_q.index(word2i["<padding>"])]
                        if word2i["<padding>"] in target_q
                        else target_q
                    )
                ...
                answer = qa["a"]
                target_q = [
                    word2i[w] if w in word2i else word2i["<unk>"] for w in q_tokens
                ]
                src_q = [word2i[start]] + [
                    word2i[w] if w in word2i else word2i["<unk>"] for w in q_tokens
                ]
                ...
                history_q_lens.extend(
                    [0] * ((max_no_qs + 1) - len(history_q_lens))
                )  # +1 because of start token is consiidered as first question
                ...
                decider_mask.extend(
                    [decidermask_pad_token] * ((max_no_qs + 1) - len_hql)
                )
                ...
                n2n_data[_id][
                    "bboxes"
                ] = bboxes  # Change in v2 only target bbox is included as everything is not required
                n2n_data[_id]["game_id"] = str(
                    game["dialogue_id"]
                )  # game_id == dialogue_id ??
            ...
            decider_mask = [0] * (len_hql - 1) + [
                n2n_data[_id]["decider_tgt"]
            ] * 1  # Because only the last target is guess
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-data_dir",
        type=str,
        default="data/test/experiments/reverse_history",
        help="Target Data Directory to store the vocab file",
    )
    parser.add_argument(
        "-data_file",
        type=str,
        default="guesswhat.test.jsonl.gz",
        help="Guesswhat train data file",
    )
    parser.add_argument(
        "-vocab_file", type=str, default="vocab.json", help="Vocabulary file"
    )
