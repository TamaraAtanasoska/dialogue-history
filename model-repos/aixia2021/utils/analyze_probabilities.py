...
if __name__ == "__main__":
    ...
    for k, v in save_guess_probs.items():
        ...
        for idx in range(1, len(v)):
            ...
            if idx == 1:
                ...
                save_target_dist[v[idx]["ans"]].append(
                    entropy(prob_dist, new_prob_dist)
                )
    ...
    for k, v in save_target_prob.items():
        print(
            "Change in probability assigned to the target object after '{}' answer:\t\t{}".format(
                k, round(np.mean(v), 3)
            )
        )
    print()
    for k, v in save_target_dist.items():
        print(
            "Change in overall probability distribution (KL divergence) after '{}' answer:\t{}".format(
                k, round(np.mean(v), 3)
            )
        )
