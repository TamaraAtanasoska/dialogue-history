...
if __name__ == "__main__":
    ...
    print(
        "\nLoading FasterRCNN features from: {}".format(
            args.load_mscoco_bottomup_train_tsv_filename
        )
    )
    mscoco_bottomup_features.extend(
        load_obj_tsv(
            args.load_mscoco_bottomup_train_tsv_filename,
            topk=1000 if args.breaking else None,
        )
    )

    print(
        "\nLoading FasterRCNN features from: {}".format(
            args.load_mscoco_bottomup_val_tsv_filename
        )
    )
    mscoco_bottomup_features.extend(
        load_obj_tsv(
    ...
    print(
        "Saving mscoco information to: {}".format(
            args.save_mscoco_bottomup_info_json_filename
        )
    )
    ...
    print(
        "Saving mscoco features to: {}".format(
            args.save_mscoco_bottomup_features_npy_filename
        )
    )
    np.save(args.save_mscoco_bottomup_features_npy_filename, mscoco_vectorized_features)

    print(
        "Saving mscoco boxes to: {}".format(
            args.save_mscoco_bottomup_boxes_npy_filename
        )
    )
