...
if __name__ == "__main__":
    ...
    parser.add_argument("-objects_features_index_path", type=str, default="./data/objects_features_index_all.json")
    parser.add_argument("-objects_features_path", type=str, default="./data/objects_features_all.h5")
    ...
    avg_img_features = np.zeros((len(games), 2048))
    game_id2pos = {}
    ...
    for game_index, game in tqdm(enumerate(games), total=len(games), ncols=50):
        ...
        for object_index, object in enumerate(game["objects"]):
            if object["id"] == game["object_id"]:
                ...
                avg_img_features[game_index] = feat.cpu().data.numpy()
                break
