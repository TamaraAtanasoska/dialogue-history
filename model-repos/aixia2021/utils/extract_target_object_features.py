...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default="./data/objects_features_index_all.json",
    )
    parser.add_argument(
        "-objects_features_path", type=str, default="./data/objects_features_all.h5"
    )
    ...
    for game_index, game in tqdm(enumerate(games), total=len(games), ncols=50):
        ...
        image = Image.open(
            os.path.join(args.image_dir, game["image"]["file_name"])
        ).convert("RGB")
