...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-config", type=str, default="config/SL/config.json", help="Config file"
    )
    ...
    if args.ckpt is not None:
        checkpoint = torch.load(
            "bin/SL/ensemble_base2022_06_24_18_53/model_ensemble_ensemble_base_E_0"
        )
    ...
    for epoch in range(optimizer_args["no_epochs"]):
            ...
            for i_batch, sample in tqdm.tqdm(
                ...
                for idx, mask in enumerate(masks):
                    ...
                    if idx == 0:
                        ...
                        word_logits_loss += _cross_entropy(
                            qgen_out.view(-1, qgen_out.shape[-1]),
                            sample["target_q"][mask].view(-1),
                        )  # TODO remove this hardcoded number

                        decider_loss += ensemble_args["decider"][
                            "ask_weight"
                        ] * decider_cross_entropy(
                            decider_out.squeeze(1), sample["decider_tgt"][mask]
                        )
                        ask_accuracy = calculate_accuracy(
                            decider_out.squeeze(1), sample["decider_tgt"][mask]
                        )
                        ...
                        if epoch % args.modulo != 0:
                            ...
                            guess_accuracy = calculate_accuracy(
                                decider_out.squeeze(1), sample["decider_tgt"][mask]
                            )

                            guesser_loss += guesser_loss_function(
                                guesser_out * sample["objects_mask"][mask].float(),
                                sample["target_obj"][mask],
                            )
                            guesser_accuracy = calculate_accuracy(
                                softmax(guesser_out),
                                sample["target_obj"][masks[1].squeeze()].reshape(-1),
                            )
                ...
                if epoch % args.modulo != 0:
                    ...
                    if args.no_decider:
                        ...
                        loss = (
                            guesser_loss + word_logits_loss + decider_loss / batch_size
                        )
                ...
                if split == "train":
                    ...
                    # Logging variables
                    if epoch % args.modulo != 0:
                        train_qgen_loss = torch.cat(
                            [train_qgen_loss, word_logits_loss.data]
                        )
                    ...
                    if epoch % args.modulo != 0:
                        val_qgen_loss = torch.cat(
                            [val_qgen_loss, word_logits_loss.data]
                        )
                        ...
                        val_guesser_loss = torch.cat(
                            [val_guesser_loss, guesser_loss.data]
                        )
        ...
        if epoch % args.modulo != 0:
            ...
            print(
                "Training Loss:: QGen %.3f, Decider %.3f"
                % (torch.mean(train_qgen_loss), torch.mean(train_decision_loss))
            )
            print(
                "Validation Loss:: QGen %.3f, Decider %.3f"
            ...
            print(
                "Training Loss:: QGen %.3f, Decider %.3f, Guesser %.3f"
                % (
                    torch.mean(train_qgen_loss),
                    torch.mean(train_decision_loss),
                    torch.mean(train_guesser_loss),
                )
            )
            print(
                "Validation Loss:: QGen %.3f, Decider %.3f, Guesser %.3f"
                % (
                    torch.mean(val_qgen_loss),
                    torch.mean(val_decision_loss),
                    torch.mean(val_guesser_loss),
                )
            )
            print(
                "Training Accuracy:: Ask %.3f, Guess  %.3f, Guesser %.3f"
                % (
                    np.mean(training_ask_accuracy),
                    np.mean(training_guess_accuracy),
                    np.mean(training_guesser_accuracy),
                )
            )
            print(
                "Validation Accuracy:: Ask %.3f, Guess  %.3f, Guesser %.3f"
                % (
                    np.mean(validation_ask_accuracy),
                    np.mean(validation_guess_accuracy),
