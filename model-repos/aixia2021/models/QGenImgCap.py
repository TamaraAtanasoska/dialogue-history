...
class QGenImgCap(nn.Module):
    ...
    def __init__(self, **kwargs):
        ...
        if self.qgen_args["visual"]:
            self.scale_visual_to = nn.Linear(
                self.qgen_args["visual_features_dim"], self.qgen_args["scale_visual_to"]
            )

            self.rnn = nn.LSTM(
                self.qgen_args["word_embedding_dim"]
                + self.qgen_args["scale_visual_to"]
                + self.qgen_args["encoder_hidden_dim"],
        ...
        self.to_logits = nn.Linear(
            self.qgen_args["hidden_dim"], self.qgen_args["vocab_size"]
        )
    ...
    def forward(self, **kwargs):
        ...
        src_q, encoder_hidden, visual_features = (
            kwargs["src_q"],
            kwargs["encoder_hidden"],
            kwargs["visual_features"],
        )
        ...
        encoder_hidden_batch = encoder_hidden.repeat(
            1, self.qgen_args["max_tgt_length"]
        ).view(batch_size, self.qgen_args["max_tgt_length"], encoder_hidden.size(1))

        if self.qgen_args["visual"]:
            proj_visual_features = self.ReLU(self.scale_visual_to(visual_features))

            # copy visual features for each input token
            proj_visual_features_batch = proj_visual_features.repeat(
                1, self.qgen_args["max_tgt_length"]
            ).view(
                batch_size,
            ...
            input = torch.cat(
                [src_q_embedding, proj_visual_features_batch, encoder_hidden_batch],
                dim=2,
            )
        else:
        ...
        word_logits = self.to_logits(
            rnn_hiddens.view(-1, self.qgen_args["hidden_dim"])
        ).view(
            batch_size, self.qgen_args["max_tgt_length"], self.qgen_args["vocab_size"]
        )
    ...
    def basicforward(self, embedding, rnn_state):
        ...
        logits = self.to_logits(
            rnn_hiddens.view(-1, self.qgen_args["hidden_dim"])
        ).view(embedding.size(0), 1, self.qgen_args["vocab_size"])
    ...
    def sampling(self, **kwargs):
        ...
        **kwargs : dict
        ...
        encoder_hidden, visual_features = (
            kwargs["encoder_hidden"],
            kwargs["visual_features"],
        )
        ...
        if self.qgen_args["visual"]:
            proj_visual_features = self.ReLU(
                self.scale_visual_to(visual_features)
            ).unsqueeze(1)
            _embedding = torch.cat(
                [start_embedding, proj_visual_features, encoder_hidden], dim=2
            )
        else:
        ...
        cell = to_var(
            torch.zeros(
                self.qgen_args["num_layers"], batch_size, self.qgen_args["hidden_dim"]
            )
        )
        hidden = to_var(
            torch.zeros(
        ...
        for i in range(self.qgen_args["max_tgt_length"]):
            if i > 0:
                ...
                if self.qgen_args["visual"]:
                    _embedding = torch.cat(
                        [word_embedding, proj_visual_features, encoder_hidden], dim=2
                    )
                else:
                    _embedding = torch.cat([word_embedding, encoder_hidden], dim=2)
            logits, rnn_state = self.basicforward(
                embedding=_embedding, rnn_state=rnn_state
            )
