...
class Oracle(nn.Module):
    ...
    def __init__(
        ...
        self.obj_categories_embedding = nn.Embedding(
            self.no_categories, self.no_category_feat
        )

        self.encoder = nn.LSTM(
            self.no_words_feat,
            self.no_hidden_encoder,
            num_layers=1,
            dropout=dropout,
            batch_first=True,
        )
        ...
        for i in range(len(self.mlp_layer_sizes) - 1):
            self.mlp.add_module(
                str(idx),
                nn.Linear(self.mlp_layer_sizes[i], self.mlp_layer_sizes[i + 1]),
            )
            idx += 1
    ...
    def forward(
        ...
        packed_question = pack_padded_sequence(
            questions_embedding, list(lengths), batch_first=True
        )
        if self.inputs_config["obj_categories"]:
        ...
        if use_cuda:
            ...
            I = Variable(
                I.expand(questions_embedding.size(0), 1, self.encoder.hidden_size) - 1
            ).cuda()
        else:
