...
def convert_sents_to_features_roberta(sents, max_seq_length, tokenizer):
    ...
    for (i, sent) in enumerate(sents):
        ...
        encodings = tokenizer.encode_plus(
            sent, add_special_tokens=True, max_length=max_seq_length, truncation=True
        )
...
class BERTEnsemble(nn.Module):
    ...
    def forward(self, **kwargs):
        ...
        **kwargs : dict
        ...
        features = convert_sents_to_features_roberta(
            history_raw, self.max_seq_length, self.tokenizer
        )

        input_ids = torch.tensor(
            [f.input_ids for f in features], dtype=torch.long, device=device
        )
        input_mask = torch.tensor(
            [f.input_mask for f in features], dtype=torch.float, device=device
        )
        ...
        encoder_output = self.encoder(
            input_ids=input_ids, attention_mask=input_mask, token_type_ids=None
        )
