...
class OracleNetwork(ResnetModel):
    def __init__(self, config, num_words, device="", reuse=False):
        ...
        with tf.variable_scope(self.scope_name, reuse=reuse) as scope:
            ...
            self._question = tf.placeholder(
                tf.int32, [self.batch_size, None], name="question"
            )
            self._seq_length = tf.placeholder(
                tf.int32, [self.batch_size], name="seq_length"
            )
            ...
            if config["inputs"]["category"]:
                self._category = tf.placeholder(
                    tf.int32, [self.batch_size], name="category"
                )
            ...
            if config["inputs"]["spatial"]:
                self._spatial = tf.placeholder(
                    tf.float32, [self.batch_size, 8], name="spatial"
                )
            ...
            # OUTPUT
            num_classes = 3
            self._answer = tf.placeholder(
                tf.float32, [self.batch_size, num_classes], name="answer"
            )

            with tf.variable_scope("mlp"):
                num_hiddens = config["model"]["MLP"]["num_hiddens"]
                l1 = utils.fully_connected(
                    emb, num_hiddens, activation="relu", scope="l1"
                )

                self.pred = utils.fully_connected(
                    l1, num_classes, activation="softmax", scope="softmax"
                )
