...
from guesswhat.models.looper.tools import (
    clear_after_stop_dialogue,
    list_to_padded_tokens,
)
...
class BasicLooper(object):
    ...
    def process(self, sess, iterator, mode, optimizer=list(), store_games=False):
        ...
        for game_data in tqdm(iterator):
            ...
            full_dialogues = [
                np.array([self.tokenizer.start_token]) for _ in range(self.batch_size)
            ]
            ...
            for no_question in range(self.max_no_question):
                ...
                (
                    ...
                    questions,
                    seq_length,
                ) = self.qgen.sample_next_question(
                    sess, prev_answers, game_data=game_data, mode=mode
                )

                # Step 1.2: Answer the question
                answers = self.oracle.answer_question(
                    sess,
                ...
                for i in range(self.batch_size):
                    full_dialogues[i] = np.concatenate(
                        (full_dialogues[i], questions[i], [answers[i]])
                    )
                ...
                if store_games:
                    padded_dialogue, seq_length = list_to_padded_tokens(
                        full_dialogues, self.tokenizer
                    )
                    _, softmax, _ = self.guesser.find_object(
                        sess, padded_dialogue, seq_length, game_data
                    )
            ...
            full_dialogues, _ = clear_after_stop_dialogue(
                full_dialogues, self.tokenizer
            )
            padded_dialogue, seq_length = list_to_padded_tokens(
                full_dialogues, self.tokenizer
            ...
            found_object, _, id_guess_objects = self.guesser.find_object(
                sess, padded_dialogue, seq_length, game_data
            )
            ...
            if store_games:
                ...
                for i, (d, g, t, f, go, po) in enumerate(
                    zip(
                        full_dialogues,
                        game_data["raw"],
                        game_data["targets_index"],
                        found_object,
                        id_guess_objects,
                        prob_objects,
                    )
    ...
    def apply_policy_gradient(
        self, sess, final_reward, padded_dialogue, seq_length, game_data, optimizer
    ):
        ...
        answer_mask = np.ones_like(
            padded_dialogue
        )  # quick and dirty mask -> TODO to improve
