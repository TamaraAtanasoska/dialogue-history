...
BeamToken = collections.namedtuple(
    "BeamToken", ["path", "word_id", "decoder_state", "score", "prev_beam"]
)
...
def create_initial_beam(decoder_state_size, batch_size=1):
    ...
    decoder_state = tf.contrib.rnn.LSTMStateTuple(
        c=np.zeros((1, decoder_state_size)), h=np.zeros((1, decoder_state_size))
    )
...
class QGenBSWrapper(object):
    ...
    def reset(self, batch_size):
        self.beam = [
            create_initial_beam(decoder_state_size=self.state_size, batch_size=1)
    ...
    def sample_next_question(self, sess, prev_answers, game_data):
        ...
        padded_questions = np.full(
            (len(self.beam), max(seq_length)), fill_value=self.tokenizer.padding_token
        )
    ...
    def eval_one_beam_search(
        self, sess, one_sample, initial_beam, keep_trajectory=False
    ):
        ...
        for depth in range(self.max_length):
            ...
            for beam_token in to_evaluate:
                ...
                if beam_token.word_id[0][-1] in [
                    self.tokenizer.stop_token,
                    self.tokenizer.stop_dialogue,
                ]:
                    memory.append(beam_token)
                    continue

                # Append a dummy STOP token to fit HQGen constraint (can also be a PADDING)
                dialogue_history = np.concatenate(
                    (beam_token.word_id, [[self.tokenizer.stop_token]]), axis=1
                )

                # evaluate next_step
                softmax, decoder_state = sess.run(
                    [self.qgen.softmax_output, self.qgen.decoder_state],
                    feed_dict={
                ...
                k_best_word_indices = np.argpartition(softmax, -self.k_best)[
                    -self.k_best :
                ]
                for word_id in k_best_word_indices:
                    memory.append(
                        BeamToken(
                            ...
                            word_id=[[word_id]],
                            decoder_state=decoder_state,
                            score=beam_token.score
                            + np.log(softmax[word_id]),  # log trick
                            prev_beam=beam_token
                            if keep_trajectory
                            else None,  # Keep trace of the previous beam if we want to keep the trajectory
                        )
                    )
