    ...
    def __init__(self, config, num_words, policy_gradient, device="", reuse=False):
        ...
        with tf.variable_scope(self.scope_name, reuse=reuse):
            ...
            self.images = tf.placeholder(
                tf.float32, [mini_batch_size] + config["image"]["dim"], name="images"
            )
            ...
            self.answer_mask = tf.placeholder(
                tf.float32, [mini_batch_size, None], name="answer_mask"
            )  # 1 if keep and (1 q/a 1) for (START q/a STOP)
            self.padding_mask = tf.placeholder(
                tf.float32, [mini_batch_size, None], name="padding_mask"
            )
            ...
            # Rewards
            self.cum_rewards = tf.placeholder(
                tf.float32, shape=[mini_batch_size, None], name="cum_reward"
            )

            # DECODER Hidden state (for beam search)
            zero_state = tf.zeros(
                [1, config["num_lstm_units"]]
            )  # default LSTM state is a zero-vector
            zero_state = tf.tile(
                zero_state, [tf.shape(self.images)[0], 1]
            )  # trick to do a dynamic size 0 tensors

            self.decoder_zero_state_c = tf.placeholder_with_default(
                zero_state, [mini_batch_size, config["num_lstm_units"]], name="state_c"
            )
            self.decoder_zero_state_h = tf.placeholder_with_default(
                zero_state, [mini_batch_size, config["num_lstm_units"]], name="state_h"
            )
            decoder_initial_state = tf.contrib.rnn.LSTMStateTuple(
                c=self.decoder_zero_state_c, h=self.decoder_zero_state_h
            )

            # Misc
            self.is_training = tf.placeholder(tf.bool, name="is_training")
            self.greedy = tf.placeholder_with_default(
                False, shape=(), name="greedy"
            )  # use for graph
            self.samples = None
            ...
            if len(config["image"]["dim"]) == 1:
                ...
                )  # TODO: improve by using the previous lstm state?

            # Reduce the embedding size of the image
            with tf.variable_scope("image_embedding"):
                self.image_emb = utils.fully_connected(
                    self.image_out, config["image_embedding_size"]
                )
            ...
            decoder_input = tf.concat(
                [input_words, image_emb], axis=2, name="concat_full_embedding"
            )
            ...
            with tf.variable_scope("decoder_output"):
                ...
                self.cross_entropy_loss = (
                    tf.nn.sparse_softmax_cross_entropy_with_logits(
                        logits=mlp_output, labels=target_words
                    )
                )
            ...
            with tf.variable_scope("ml_loss"):
                ...
                ml_loss *= self.answer_mask[
                    :, 1:
                ]  # remove answers (ignore the <stop> token)
                ml_loss *= self.padding_mask[
                    :, 1:
                ]  # remove padding (ignore the <start> token)

                # Count number of unmask elements
                count = (
                    tf.reduce_sum(self.padding_mask)
                    - tf.reduce_sum(1 - self.answer_mask[:, :-1])
                    - 1
                )  # no_unpad - no_qa - START token

                ml_loss = tf.reduce_sum(
                    ml_loss, axis=1
                )  # reduce over dialogue dimension
                ml_loss = tf.reduce_sum(
                    ml_loss, axis=0
                )  # reduce over minibatch dimension
            ...
            if policy_gradient:
                ...
                with tf.variable_scope("rl_baseline"):
                    decoder_out = tf.stop_gradient(
                        self.decoder_output
                    )  # take the LSTM output (and stop the gradient!)

                    flat_decoder_output = tf.reshape(
                        decoder_out, [-1, decoder_lstm_cell.output_size]
                    )  #
                    flat_h1 = utils.fully_connected(
                        flat_decoder_output,
                        n_out=100,
                        activation="relu",
                        scope="baseline_hidden",
                    )
                    flat_baseline = utils.fully_connected(
                        flat_h1, 1, activation="relu", scope="baseline_out"
                    )

                    self.baseline = tf.reshape(
                        flat_baseline, [tf.shape(self.seq_length)[0], max_sequence - 1]
                    )
                ...
                with tf.variable_scope("policy_gradient_loss"):
                    ...
                    self.log_of_policy *= self.answer_mask[
                        :, 1:
                    ]  # remove answers (<=> predicted answer has maximum reward) (ignore the START token in the mask)
                    ...
                    self.score_function = tf.multiply(
                        self.log_of_policy, rewards - self.baseline
                    )  # score function

                    self.baseline_loss = tf.reduce_sum(
                        tf.square(rewards - self.baseline)
                    )

                    self.policy_gradient_loss = tf.reduce_sum(
                        self.score_function, axis=1
                    )  # sum over the dialogue trajectory
                    self.policy_gradient_loss = tf.reduce_mean(
                        self.policy_gradient_loss, axis=0
                    )  # reduce over minibatch dimension
    ...
    def build_sampling_graph(self, config, tokenizer, max_length=12):
        ...
        def stop_cond(states_c, states_h, tokens, seq_length, stop_indicator):
            ...
            has_unfinished_dialogue = tf.less(
                tf.shape(tf.where(stop_indicator))[0], tf.shape(stop_indicator)[0]
            )  # TODO use "any" instead of checking shape
        ...
        def step(prev_state_c, prev_state_h, tokens, seq_length, stop_indicator):
            ...
            stop_indicator = tf.logical_or(
                stop_indicator, is_stop
            )  # flag to false new finished dialogue
            ...
            with tf.variable_scope(self.scope_name, reuse=True):
                ...
                with tf.variable_scope("decoder_output"):
                    output = utils.fully_connected(
                        state_h, tokenizer.no_words, reuse=True
                    )
            ...
            tokens = tf.concat(
                [tokens, tf.expand_dims(sampled_tokens, 0)], axis=0
            )  # check axis!
