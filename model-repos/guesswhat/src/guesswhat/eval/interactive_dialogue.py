...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-img_dir", type=str, help="Directory with images to feed networks"
    )
    parser.add_argument(
        "-img_raw_dir", type=str, help="Directory with images to display"
    )
    parser.add_argument("-crop_dir", type=str, help="Directory with crops")
    parser.add_argument(
        "-exp_dir", type=str, required=False, help="Directory to output dialogue"
    )
    ...
    parser.add_argument(
        "-oracle_identifier",
        type=str,
        default="156cb3d352b97ba12ffd6cf547281ae2",
        required=False,
        help="Oracle identifier - if none: user must be the oracle",
    )  # Use checkpoint id instead?
    parser.add_argument(
        "-qgen_identifier",
        type=str,
        default="7b24d8b68f94bde9774cd9555584fd93",
        required=False,
        help="Qgen identifier - if none: user must be the Qgen",
    )
    parser.add_argument(
        "-guesser_identifier",
        type=str,
        required=False,
        help="Guesser identifier - if none: user must be the guesser",
    )
    ...
    with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options)) as sess:
        ...
        if args.guesser_identifier is not None:
            ...
            guesser_wrapper = GuesserUserWrapper(
                tokenizer, img_raw_dir=args.img_raw_dir
            )
        ...
        while True:
            ...
            while True:
                ...
                if id_str == "y" or id_str == "yes":
                    break
                elif id_str == "n" or id_str == "no":
                    exit(0)
            ...
            while True:
                id_str = input(
                    "Which object id do you want to select? (-1 for random id) -->  "
                )
                ...
                if 0 <= id_str < len(game.objects):
                    ...
                    print(
                        "Could not find the following object index: {}".format(id_str)
                    )
