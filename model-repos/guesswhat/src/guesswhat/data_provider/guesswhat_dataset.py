...
class Game:
    ...
    def show(self, img_raw_dir, display_index=False, display_mask=False):
        ...
        for i, obj in enumerate(self.objects):
            if display_index:
                draw.text(
                    (obj.bbox.x_center, self.image.height - obj.bbox.y_center), str(i)
                )
            if display_mask:
...
class Image:
    def __init__(self, id, width, height, url, which_set, image_builder=None):
        ...
        if image_builder is not None:
            ...
            self.image_loader = image_builder.build(
                id, which_set=which_set, filename=self.filename, optional=False
            )
...
class Bbox:
    ...
    def __str__(self):
        return "center : {0:5.2f}/{1:5.2f} - size: {2:5.2f}/{3:5.2f}".format(
            self.x_center, self.y_center, self.x_width, self.y_height
        )
...
class Object:
    def __init__(
        ...
        if crop_builder is not None:
            ...
            self.crop_loader = crop_builder.build(
                id, filename=filename, which_set=which_set, bbox=bbox
            )
    ...
    with gzip.open(save_path.format("guesswhat." + name + ".jsonl.gz"), "wb") as f:
        for id_game, d in enumerate(data):
            ...
            for k, word in enumerate(dialogue):
                if (
                    word == tokenizer.yes_token
                    or word == tokenizer.no_token
                    or word == tokenizer.non_applicable_token
                ):
                    ...
                    prob_obj = [
                        str(round(p, 3)) for p in prob_obj
                    ]  # decimal are not supported y default in json encoder

                    qas.append(
                        {"question": q, "answer": a[1:-1], "id": 0, "p": prob_obj}
                    )
