...
from generic.data_provider.image_preprocessors import (
    resize_image,
    get_spatial_feat,
    scaled_crop_and_pad,
)
        ...
        for i, game in enumerate(games):
            ...
            for index, obj in enumerate(game.objects):
                spatial = get_spatial_feat(
                    obj.bbox, game.image.width, game.image.height
                )
            ...
            if img is not None:
                if (
                    "images" not in batch
                ):  # initialize an empty array for better memory consumption
