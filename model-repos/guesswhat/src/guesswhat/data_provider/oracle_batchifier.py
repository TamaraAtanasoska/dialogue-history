    ...
    def apply(self, games):
        ...
        for i, game in enumerate(games):
            ...
            if "spatial" in sources:
                spat_feat = get_spatial_feat(
                    game.object.bbox, image.width, image.height
                )
            ...
            if "mask" in sources:
                ...
                ft_width, ft_height = (
                    batch["image"][-1].shape[1],
                    batch["image"][-1].shape[2],
                )  # Use the image feature size (not the original img size)
                mask = resize_image(
                    Image.fromarray(mask), height=ft_height, width=ft_width
                )
        ...
        if "question" in sources:
            batch["question"], batch["seq_length"] = padder(
                batch["question"], padding_symbol=tokenizer.word2i["<padding>"]
            )
