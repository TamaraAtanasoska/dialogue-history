...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-data_dir", type=str, required=True, help="Directory with data"
    )
    parser.add_argument(
        "-exp_dir",
        type=str,
        required=True,
        help="Directory in which experiments are stored",
    )
    parser.add_argument("-img_dir", type=str, help="Directory with images")
    parser.add_argument("-crop_dir", type=str, help="Directory with images")
    parser.add_argument("-config", type=str, required=True, help="Config file")
    parser.add_argument(
        "-dict_file", type=str, default="dict.json", help="Dictionary file name"
    )

    parser.add_argument(
        "-networks_dir", type=str, help="Directory with pretrained networks"
    )
    parser.add_argument(
        "-oracle_identifier", type=str, required=True, help="Oracle identifier"
    )  # Use checkpoint id instead?
    parser.add_argument(
        "-qgen_identifier", type=str, required=True, help="Qgen identifier"
    )
    parser.add_argument(
        "-guesser_identifier", type=str, required=True, help="Guesser identifier"
    )
    ...
    parser.add_argument(
        ...
        type=lambda x: bool(strtobool(x)),
        default="False",
        help="Evaluate sampling, greedy and BeamSearch?",
    )  # TODO use an input list
    parser.add_argument(
        "-store_games",
        type=lambda x: bool(strtobool(x)),
        default="True",
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.95,
        help="How muany GPU ram is required? (ratio)",
    )
    parser.add_argument(
        "-no_thread", type=int, default=1, help="No thread to load batch"
    )
    ...
    oracle_config = get_config_from_xp(
        os.path.join(args.networks_dir, "oracle"), args.oracle_identifier
    )
    guesser_config = get_config_from_xp(
        os.path.join(args.networks_dir, "guesser"), args.guesser_identifier
    )
    ...
    if oracle_config["inputs"].get("crop", False):
        ...
        crop_builder = get_img_builder(
            oracle_config["model"]["crop"], args.crop_dir, is_crop=True
        )
    ...
    qgen_network = QGenNetworkLSTM(
        qgen_config["model"], num_words=tokenizer.no_words, policy_gradient=True
    )
    qgen_var = [
        v for v in tf.global_variables() if "qgen" in v.name
    ]  # and 'rl_baseline' not in v.name
    ...
    guesser_network = GuesserNetwork(
        guesser_config["model"], num_words=tokenizer.no_words
    )
    ...
    baseline_variables = [
        v
        for v in tf.trainable_variables()
        if "qgen" in v.name and "rl_baseline" in v.name
    ]

    pg_optimize, _ = create_optimizer(
        qgen_network,
        loop_config["optimizer"],
        var_list=pg_variables,
        optim_cst=tf.train.GradientDescentOptimizer,
        loss=qgen_network.policy_gradient_loss,
    )
    baseline_optimize, _ = create_optimizer(
        qgen_network,
        loop_config["optimizer"],
    ...
    with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options)) as sess:
        ...
        if args.continue_exp:  # TODO only reload qgen ckpt
            ...
            qgen_loader_supervized.restore(
                sess,
                os.path.join(
                    args.networks_dir, "qgen", args.qgen_identifier, "params.ckpt"
                ),
            )

        oracle_saver.restore(
            sess,
            os.path.join(
                args.networks_dir, "oracle", args.oracle_identifier, "params.ckpt"
            ),
        )
        guesser_saver.restore(
            sess,
            os.path.join(
                args.networks_dir, "guesser", args.guesser_identifier, "params.ckpt"
            ),
        )
        ...
        for epoch in range(no_epoch):
            ...
            train_score = looper_evaluator.process(
                sess, train_iterator, optimizer=optimizer, mode="sampling"
            )
