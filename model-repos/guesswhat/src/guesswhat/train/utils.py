...
def test_oracle(sess, testset, tokenizer, oracle, cpu_pool, batch_size, logger):
    ...
    oracle_evaluator = Evaluator(
        oracle_sources, oracle.scope_name, network=oracle, tokenizer=tokenizer
    )
    oracle_batchifier = OracleBatchifier(tokenizer, oracle_sources, status=("success",))
    oracle_iterator = Iterator(
        oracle_dataset,
        pool=cpu_pool,
        batch_size=batch_size,
        batchifier=oracle_batchifier,
    )
    [oracle_loss, oracle_error] = oracle_evaluator.process(
        sess, oracle_iterator, [oracle.loss, oracle.error]
    )
...
def test_guesser(sess, testset, tokenizer, guesser, cpu_pool, batch_size, logger):
    ...
    guesser_evaluator = Evaluator(
        guesser_sources, guesser.scope_name, network=guesser, tokenizer=tokenizer
    )
    guesser_batchifier = QuestionerBatchifier(
        tokenizer, guesser_sources, status=("success",)
    )
    guesser_iterator = Iterator(
        testset, pool=cpu_pool, batch_size=batch_size, batchifier=guesser_batchifier
    )
    [guesser_loss, guesser_error] = guesser_evaluator.process(
        sess, guesser_iterator, [guesser.loss, guesser.error]
    )
...
def test_qgen(sess, testset, tokenizer, qgen, cpu_pool, batch_size, logger):
    ...
    qgen_evaluator = Evaluator(
        qgen_sources, qgen.scope_name, network=qgen, tokenizer=tokenizer
    )
    qgen_batchifier = QuestionerBatchifier(tokenizer, qgen_sources, status=("success",))
    qgen_iterator = Iterator(
        testset, pool=cpu_pool, batch_size=batch_size, batchifier=qgen_batchifier
    )
...
def test_model(
    sess, testset, tokenizer, oracle, guesser, qgen, cpu_pool, batch_size, logger
):
...
def compute_qgen_accuracy(
    ...
    for m in mode:
        ...
        test_score = evaluator.process(
            sess, test_iterator, mode=m, store_games=store_games
        )

        # Retrieve the generated games and dump them as a dataset
        if store_games:
            generated_dialogues = evaluator.get_storage()
            dump_samples_into_dataset(
                generated_dialogues,
                save_path=save_path,
                tokenizer=tokenizer,
                name=dump_suffix + "." + m,
