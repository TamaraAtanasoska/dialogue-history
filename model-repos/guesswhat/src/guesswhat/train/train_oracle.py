...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-exp_dir", type=str, help="Directory in which experiments are stored"
    )
    ...
    if config["inputs"].get("crop", False):
        ...
        crop_builder = get_img_builder(
            config["model"]["crop"], args.crop_dir, is_crop=True
        )
    ...
    with tf.Session(
        ...
        if use_resnet:
            resnet_saver.restore(
                sess,
                os.path.join(args.data_dir, "resnet_v1_{}.ckpt".format(resnet_version)),
            )
        ...
        for t in range(start_epoch, no_epoch):
            ...
            train_loss, train_accuracy = evaluator.process(
                sess, train_iterator, outputs=outputs + [optimizer]
            )
            ...
            valid_loss, valid_accuracy = evaluator.process(
                sess, valid_iterator, outputs=outputs
            )
