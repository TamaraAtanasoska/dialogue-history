...
parser = argparse.ArgumentParser(
    description="Python make_cococaption_id_names.py {cococaption basedir}\n\n"
...
parser.add_argument(
    ...
    type=str,
    required=True,
    help="Input Image folder (WARNING absolute path)",
)
parser.add_argument(
    "-image_subdir",
    type=list,
    default=["train2014", "val2014", "test2014"],
...
parser.add_argument(
    ...
    type=str,
    required=True,
    help="Output symlink folder (WARNING absolute path)",
)

args = parser.parse_args()

assert args.image_dir.startswith(os.path.sep), "The path must be a root path: ".format(
    args.image_dir
)
assert args.data_out.startswith(os.path.sep), "The path must be a root path: ".format(
    args.data_out
)
