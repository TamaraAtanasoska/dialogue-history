...
parser.add_argument(
    "-set_type",
    type=list,
    default=["valid", "train", "test"],
    help="Select the dataset to dump",
)

parser.add_argument("-out_dir", type=str, required=True, help="Output folder")

parser.add_argument(
    "-network", type=str, choices=["resnet", "vgg"], help="Use resnet/vgg network"
)
parser.add_argument(
    "-resnet_version",
    type=int,
    default=152,
    choices=[50, 101, 152],
    help="Pick the resnet version [50/101/152]",
)
parser.add_argument(
    "-ckpt", type=str, required=True, help="Path for network checkpoint: "
)
parser.add_argument(
    "-feature_name",
    type=str,
    default="",
    help="Pick the name of the network features default=(fc8 - block4)",
)
...
parser.add_argument(
    ...
    type=str,
    choices=["img", "crop"],
    help="Select to either dump the img/crop feature",
)
parser.add_argument(
    "-subtract_mean",
    type=lambda x: bool(strtobool(x)),
    default="True",
    help="Preprocess the image by substracting the mean",
)
parser.add_argument("-img_size", type=int, default=224, help="image size (pixels)")
parser.add_argument(
    "-crop_scale", type=float, default=1.1, help="crop scale around the bbox"
)
parser.add_argument(
    "-batch_size", type=int, default=64, help="Batch size to extract features"
)

parser.add_argument(
    "-gpu_ratio", type=float, default=0.95, help="How many GPU ram is required? (ratio)"
)
...
if args.mode == "img":
    images = tf.placeholder(
        tf.float32, [None, args.img_size, args.img_size, 3], name="image"
    )
    source = "image"
    crop_builder = None
    dataset_cstor = OracleDataset.load
    image_builder = RawImageBuilder(
        args.img_dir, height=args.img_size, width=args.img_size, channel=channel_mean
    )

elif args.mode == "crop":
    images = tf.placeholder(
        tf.float32, [None, args.img_size, args.img_size, 3], name="crop"
    )
    source = "crop"
    image_builder = None
    dataset_cstor = CropDataset.load
    crop_builder = RawCropBuilder(
        args.img_dir,
...
if args.network == "resnet":
    ft_output = resnet.create_resnet(
        images,
