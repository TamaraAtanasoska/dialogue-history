...
class CBNAbtract(object):
    ...
    def create_cbn_input(self, feature_maps):
        ...
        delta_betas = tf.zeros(
            shape=[batch_size, feat]
        )  # Note that this does not compile (batch_size=None)
...
class CBNfromLSTM(CBNAbtract):
    ...
    def create_cbn_input(self, feature_maps):
        ...
        if self.use_betas:
            ...
            delta_betas = tf.tile(
                tf.constant(0.0, shape=[1, no_features]), tf.stack([batch_size, 1])
            )

        if self.use_gammas:
            h_gammas = utils.fully_connected(
                self.lstm_state,
                self.cbn_embedding_size,
                weight_initializer=RandomUniform(-1e-4, 1e-4),
                scope="hidden_gammas",
