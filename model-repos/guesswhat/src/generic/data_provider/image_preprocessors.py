...
def get_spatial_feat(bbox, im_width, im_height):
    ...
    x_left = (1.0 * bbox.x_left / im_width) * 2 - 1
    x_right = (1.0 * bbox.x_right / im_width) * 2 - 1
    x_center = (1.0 * bbox.x_center / im_width) * 2 - 1

    y_lower = (1.0 * bbox.y_lower / im_height) * 2 - 1
    y_upper = (1.0 * bbox.y_upper / im_height) * 2 - 1
    y_center = (1.0 * bbox.y_center / im_height) * 2 - 1

    x_width = (1.0 * bbox.x_width / im_width) * 2
    y_height = (1.0 * bbox.y_height / im_height) * 2
...
def scaled_crop_and_pad(bbox, raw_img, scale=1.0):
    ...
    background.paste(
        im=crop, box=(((max_side - crop_w) // 2), (max_side - crop_h) // 2)
    )
    return background
