...
def get_image_features(
    image, question, is_training, scope_name, config, dropout_keep=1.0, reuse=False
):
    image_input_type = config["image_input"]

    # Extract feature from 1D-image feature s
    if (
        image_input_type == "fc8"
        ...
        if image_input_type.startswith("raw"):
            ...
            image_feature_maps = create_resnet(
                image,
                is_training=is_training,
                scope=scope_name,
                cbn=cbn,
            ...
            if config.get("normalize", False):
                image_feature_maps = tf.nn.l2_normalize(
                    image_feature_maps, dim=[1, 2, 3]
                )
