...
class AbstractNetwork(object):
    ...
    def get_sources(self, sess):
        return [
            os.path.basename(tensor.name)
            for tensor in self.get_inputs(sess)
            if self.scope_name in tensor.name
        ]

    def get_inputs(self, sess):
        placeholders = [
            p
            for p in sess.graph.get_operations()
            if "holder" in p.type
            if self.scope_name in p.name
        ]
        if self.device is not "":
            return [
                p for p in placeholders if p.device[-1] == str(self.device)
            ]  # use for multi-gpu computation
        return placeholders
...
class ResnetModel(AbstractNetwork):
    ...
    def get_parameters(self, finetune=list()):
        ...
        params = [
            v for v in params if (not "resnet" in v.name or "cbn_input" in v.name)
        ]
        ...
        if len(finetune) > 0:
            for e in finetune:
                fine_tuned_params = [
                    ...
                    if e in v.name and v not in params
                ]
    ...
    def get_resnet_parameters(self):
        ...
        return [
            ...
            if self.scope_name in v.name
            and (
                "resnet" in v.name
                and "cbn_input" not in v.name
                and "Adam" not in v.name
                and "local_step" not in v.name
                and "moving_mean/biased" not in v.name
            )
        ]
