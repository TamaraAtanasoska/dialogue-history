...
class Evaluator(object):
    def __init__(
        self, provided_sources, scope="", writer=None, network=None, tokenizer=None
    ):  # debug purpose only, do not use in the code
    ...
    def process(self, sess, iterator, outputs, listener=None):
        ...
        if listener is not None:
            ...
            outputs = list(
                OrderedDict.fromkeys(outputs)
            )  # remove duplicate while preserving ordering
            listener.before_epoch(is_training)

        n_iter = 1.0
        aggregated_outputs = [
            0.0 for v in outputs if is_scalar(v) and v in original_outputs
        ]
        ...
        for batch in tqdm(iterator):
            ...
            for var, result in zip(outputs, results):
                if is_scalar(var) and var in original_outputs:
                    # moving average
                    aggregated_outputs[i] = (
                        (n_iter - 1.0) / n_iter
                    ) * aggregated_outputs[i] + result / n_iter
                    i += 1
    ...
    def execute(self, sess, output, batch):
        feed_dict = {
            self.scope + key + ":0": value
            for key, value in batch.items()
            if key in self.provided_sources
        }
...
class MultiGPUEvaluator(object):
    ...
    def __init__(
        self, provided_sources, name_scopes, writer=None, networks=None, tokenizer=None
    ):  # Debug purpose only, do not use here
    ...
    def process(self, sess, iterator, outputs, listener=None):
        ...
        assert (
            listener is None
        ), "Listener are not yet supported with multi-gpu evaluator"
        assert isinstance(outputs, list), "outputs must be a list"
